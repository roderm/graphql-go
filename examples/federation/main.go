...
import (
	...
	"github.com/graphql-go/graphql/examples/federation/model"
	"github.com/graphql-go/graphql/examples/federation/resolver"
...
func main() {
	schema, _ := federation.NewFederatedSchema(federation.FederatedSchemaConfig{
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		result := executeQuery(request.Query, request.Variables, schema)
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(result)
	})

	fmt.Println("Now server is running on port 4001")
